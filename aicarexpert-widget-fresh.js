...
console.log('🚀 FRESH WIDGET: Script execution started!');
...
window.AiCareXpert = {
  ...
  sendTestMessage: function(message) {
    console.log('🧪 FRESH WIDGET Test:', message);
    alert('FRESH WIDGET Test: ' + message);
    return 'Test successful';
  },
  
  getConfig: function() {
    console.log('📋 FRESH WIDGET: Getting config');
    return this.config || { status: 'No config set' };
  },
  
  init: function(options) {
    console.log('🔧 FRESH WIDGET: Initializing with options:', options);
    ...
    console.log('🔧 FRESH WIDGET: Config stored:', this.config);
    this.createWidget();
    console.log('✅ FRESH WIDGET: Widget initialized successfully');
    return 'Widget initialized';
  },
  
  createWidget: function() {
    console.log('🎨 FRESH WIDGET: Creating widget UI');
    ...
    console.log('✅ FRESH WIDGET: Widget UI created with event listeners');
  },
  
  setupEventListeners: function() {
    console.log('🔗 FRESH WIDGET: Setting up event listeners');
    ...
    if (input) {
      input.addEventListener('keypress', (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {
          e.preventDefault();
          console.log('⌨️ FRESH WIDGET: Enter key pressed');
          this.sendMessage();
        }
      });
      console.log('✅ FRESH WIDGET: Input event listener added');
    } else {
      console.error('❌ FRESH WIDGET: Input element not found for event listener');
    }
    
    if (sendButton) {
      sendButton.addEventListener('click', (e) => {
        e.preventDefault();
        console.log('🖱️ FRESH WIDGET: Send button clicked');
        this.sendMessage();
      });
      console.log('✅ FRESH WIDGET: Send button event listener added');
    } else {
      console.error('❌ FRESH WIDGET: Send button not found for event listener');
    }
  },
  
  toggleWidget: function() {
    console.log('🔄 FRESH WIDGET: Toggling widget');
  ...
  addMessage: function(content, role) {
    console.log(`💬 FRESH WIDGET: Adding ${role} message:`, content);
    const messagesContainer = document.getElementById(this.config.widgetId + '-messages');
    if (!messagesContainer) {
      console.error('❌ FRESH WIDGET: Messages container not found');
      return;
    }
    ...
    console.log('✅ FRESH WIDGET: Message added to UI');
  },
  
  showTyping: function() {
    console.log('⏳ FRESH WIDGET: Showing typing indicator');
  ...
  hideTyping: function() {
    console.log('⏹️ FRESH WIDGET: Hiding typing indicator');
    const typingEl = document.getElementById(this.config.widgetId + '-typing');
    if (typingEl) typingEl.style.display = 'none';
  },
  
  sendMessage: async function() {
    console.log('🚀 FRESH WIDGET: sendMessage function called!');
    
    const input = document.getElementById(this.config.widgetId + '-input');
    const sendButton = document.getElementById(this.config.widgetId + '-send');
    
    if (!input) {
      console.error('❌ FRESH WIDGET: Input element not found');
      return;
    }
    
    if (!sendButton) {
      console.error('❌ FRESH WIDGET: Send button not found');
      return;
    }
    
    const message = input.value.trim();
    if (!message) {
      console.log('❌ FRESH WIDGET: Empty message, not sending');
      return;
    }
    
    console.log('📤 FRESH WIDGET: Sending message:', message);
    ...
    try {
      console.log('🌐 FRESH WIDGET: Making API call to:', this.config.apiUrl);
      console.log('🔑 FRESH WIDGET: Using tenant ID:', this.config.tenantId);
      console.log('🤖 FRESH WIDGET: Using assistant ID:', this.config.assistantId);
      ...
      console.log('📦 FRESH WIDGET: Request body:', requestBody);
      ...
      console.log('📡 FRESH WIDGET: API response status:', response.status);
      console.log('📡 FRESH WIDGET: API response headers:', [...response.headers.entries()]);
      
      if (response.ok) {
        const data = await response.json();
        console.log('✅ FRESH WIDGET: API response data:', data);
        
        // Update session info
        if (data.sessionId) {
          this.sessionId = data.sessionId;
          console.log('🔄 FRESH WIDGET: Session ID updated:', this.sessionId);
        }
        if (data.userId) {
          this.userId = data.userId;
          console.log('🔄 FRESH WIDGET: User ID updated:', this.userId);
        }
        
        // Add assistant response
        this.addMessage(data.response, 'assistant');
        console.log('✅ FRESH WIDGET: Assistant response added to chat');
      } else {
        const errorText = await response.text();
        console.error('❌ FRESH WIDGET: API error:', response.status, errorText);
        this.addMessage('Sorry, I encountered an error. Please try again.', 'assistant');
      }
    } catch (error) {
      console.error('❌ FRESH WIDGET: Network error:', error);
      this.hideTyping();
      ...
      input.focus();
      console.log('🔄 FRESH WIDGET: Send button re-enabled');
    }
  }
};

console.log('✅ FRESH WIDGET: Object created with properties:', Object.keys(window.AiCareXpert));
console.log('✅ FRESH WIDGET: Script execution completed successfully!');