...
window.AiCareXpert = {
  ...
  createWidget: function() {
    ...
    widget.innerHTML = `
      ...
      ">
        <!-- Chat Button -->
        <button id="${this.config.widgetId}-button" onclick="window.AiCareXpert.toggleWidget()" style="
          width: 60px;
          height: 60px;
        ...
        ">
          ...
          ">
            <input 
              ...
              onkeypress="if(event.key==='Enter' && !event.shiftKey) { event.preventDefault(); window.AiCareXpert.sendMessage(); }"
              style="
                flex: 1;
                padding: 10px 12px;
            ...
            />
            <button 
              onclick="event.preventDefault(); window.AiCareXpert.sendMessage();"
  ...
  sendMessage: async function() {
    console.log('🚀 AiCareXpert: sendMessage called');
    
    const input = document.getElementById(this.config.widgetId + '-input');
    const sendButton = document.getElementById(this.config.widgetId + '-send');
    
    if (!input || !sendButton) {
      console.error('❌ AiCareXpert: Input or send button not found');
      return;
    }
...
console.log('✅ AiCareXpert Widget: Loaded successfully with properties:', Object.keys(window.AiCareXpert));